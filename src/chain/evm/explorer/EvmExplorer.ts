...
import { hexToNumber } from '../../../foundation/encoding/hex';
import type {
    EvmAddressInfo,
    EvmBlock,
    EvmLog,
...
export class EvmExplorer {
    ...
    async getAddressInfo(address: string): Promise<EvmAddressInfo> {
        ...
        const [balance, nonce, code, logs] = await Promise.all([
            ...
            this.evm
                .request<RawEvmLog[] | null>('eth_getLogs', [
                    { fromBlock, toBlock: 'latest', address },
                ])
                .catch(() => []),
        ]);
    ...
    async getTokenTransfers(address: string, holderAddress?: string): Promise<TokenTransfer[]> {
        ...
        if (holderAddress) {
            ...
            const [sent, received] = await Promise.all([
                this.evm
                    .request<RawEvmLog[]>('eth_getLogs', [
                        { fromBlock, toBlock: 'latest', address, topics: [TRANSFER, padded] },
                    ])
                    .catch(() => [] as RawEvmLog[]),
                this.evm
                    .request<RawEvmLog[]>('eth_getLogs', [
                        { fromBlock, toBlock: 'latest', address, topics: [TRANSFER, null, padded] },
                    ])
                    .catch(() => [] as RawEvmLog[]),
            ]);
            ...
            const all = await this.evm
                .request<RawEvmLog[]>('eth_getLogs', [
                    { fromBlock, toBlock: 'latest', address, topics: [TRANSFER] },
                ])
                .catch(() => [] as RawEvmLog[]);
            logs = (all ?? []).slice(-100).toReversed();
        }
