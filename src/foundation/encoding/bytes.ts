...
/**
 * The on-chain hex form of a commitment — the inverse of {@link leHexToBigint}.
 *
 * A commitment is a field element, but everything that INDEXES one — scan
 * hints, vault records, a note's `commitmentHex` — uses its 32-byte LITTLE-ENDIAN
 ...
 *
 * One function so the encoding is stated once — and public, beside its inverse,
 * because verifying a disclosure needs it: `decodeNoteDisclosureKey` hands back
 * a `commitment` bigint, and comparing that against what an indexer serves means
 * encoding it exactly this way.
 */
