...
vi.mock('@polkadot-api/substrate-bindings', () => ({
    ...
    AccountId: vi.fn().mockReturnValue({
        dec: vi.fn().mockReturnValue('5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY'),
    }),
}));
...
import type { EventRecord } from '../../../src/chain/substrate/types/index';
...
function makeBuilderMock(decResult: unknown[] = []) {
    ...
    const enc = vi
        .fn()
        .mockReturnValue('0x26aa394eea5630e07c48ae0c9558cef780d41e5e16056765bc8461851072c9d7');
...
describe('SubstrateClient.connect', () => {
    ...
    it('throws on connection timeout', async () => {
        ...
        await expect(SubstrateClient.connect('ws://localhost', 1)).rejects.toThrow(
            'Connection timeout (1ms) to ws://localhost'
        );
    });
    ...
    it('timeout message includes wsUrl and duration', async () => {
        ...
        await expect(SubstrateClient.connect('ws://my-node.example.com', 1)).rejects.toThrow(
            'ws://my-node.example.com'
        );
    });
});
...
describe('SubstrateClient.txFromCallData', () => {
    it('calls unsafeApi.txFromCallData with the callData directly', async () => {
        const mockTx = { createAndSubmit: vi.fn() };
...
describe('SubstrateClient.submit', () => {
    it('calls _papi.submit with the signed hex', async () => {
        ...
        expect(papi.submit).toHaveBeenCalledWith(
            expect.objectContaining({ _tag: 'Binary', hex: '0xsignedtx' })
        );
    });
...
describe('SubstrateClient.submitAndWatch', () => {
    it('calls _papi.submitAndWatch with the signed hex', async () => {
        ...
        expect(papi.submitAndWatch).toHaveBeenCalledWith(
            expect.objectContaining({ _tag: 'Binary', hex: '0xsignedtx' })
        );
    });
...
describe('SubstrateClient.signAndSubmit', () => {
    it('builds the tx from callData and signs+submits with the signer', async () => {
        ...
        const mockTx = { createAndSubmit: mockSignAndSubmit };
        const unsafeApi = { txFromCallData: vi.fn().mockResolvedValue(mockTx) };
        const { client } = await makeClient({
            getUnsafeApi: vi.fn().mockReturnValue(unsafeApi),
        });
    ...
    it('propagates errors from createAndSubmit', async () => {
...
describe('SubstrateClient.queryBlockEvents', () => {
    ...
    it('calls state_getStorage with the encoded key and blockHash', async () => {
        ...
        expect(papi._request).toHaveBeenCalledWith('state_getStorage', [
            enc.mock.results[0]?.value,
            BLOCK_HASH,
        ]);
    });
...
describe('SubstrateClient.extractAuthorFromLogs', () => {
    ...
    it('decodes mode-1 compact length', () => {
        vi.mocked(AccountId).mockReturnValueOnce({
            dec: vi.fn().mockReturnValue('5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty'),
        } as never);
    ...
    it('decodes mode-2 compact length', () => {
        vi.mocked(AccountId).mockReturnValueOnce({
            dec: vi.fn().mockReturnValue('5DAAnrj7VHTznn2AWBemMuyBwZWs6FNFjdyVXUeYum3PTXFy'),
        } as never);
...
// ─── adopt(): sharing the caller's PAPI client ───────────────────────────────
//
// A dApp with its own connection manager would otherwise get a second
// WebSocket and a second view of chain state, because connect() builds its
// provider internally. The ownership rule is what makes sharing safe: an
// adopted client is the caller's, and disposing the SDK object must not close
// a connection the rest of their application is still using.

describe('SubstrateClient.adopt', () => {
    it('wraps an existing client without opening a connection', async () => {
        const papi = makeMockPapi();

        const client = SubstrateClient.adopt(papi as never);
        await client.request('system_name');

        expect(papi._request).toHaveBeenCalledWith('system_name', []);
        // connect() would have called getWsProvider; adopt() must not.
        expect(vi.mocked(getWsProvider)).not.toHaveBeenCalled();
    });

    it('leaves an adopted client running on destroy', () => {
        const papi = makeMockPapi();

        SubstrateClient.adopt(papi as never).destroy();

        // The single most important assertion here: closing this would take the
        // host application's connection down with it.
        expect(papi.destroy).not.toHaveBeenCalled();
    });

    it('still closes a connection it opened itself', async () => {
        const papi = makeMockPapi();
        vi.mocked(createClient).mockReturnValue(papi as never);
        ...
        expect(papi.destroy).toHaveBeenCalled();
    });

    it('rejects batchRequest without an HTTP endpoint, and allows it with one', async () => {
        // An adopted transport may not be a WebSocket at all, so there is nothing
        // to derive the HTTP URL from — say so rather than fetch `null`.
        await expect(
            SubstrateClient.adopt(makeMockPapi() as never).batchRequest([
                { method: 'x', params: [] },
            ])
